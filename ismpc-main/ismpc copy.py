...
class Ismpc:
  def __init__(self, initial, footstep_planner, params):
    ...
    self.x0_param = self.opt.parameter(9)#Define the number of parameter
    ...
    # The solver must obtain the number of solutions equals to self.N (a defined length of MPC)
    for i in range(self.N):
      self.opt.subject_to(self.X[:, i + 1] == self.X[:, i] + self.delta * self.f(self.X[:, i], self.U[:, i]))
    # The cost function is defined in the paper "... world of stairs"
  ...
  def solve(self, current, t):
    ...
    sol = self.opt.solve()
    # In MPC: apply only first predicted output to the system
    self.x = sol.value(self.X[:,1])
    self.u = sol.value(self.U[:,0])
    # Set initial value for the next step!?
