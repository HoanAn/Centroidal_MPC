...
class Hrp4Controller(dart.gui.osg.RealTimeWorldNode):
    def __init__(self, world, hrp4):
        ...
        # set initial configuration (joint space)
        initial_configuration = {'CHEST_P': 0., 'CHEST_Y': 0., 'NECK_P': 0., 'NECK_Y': 0., \
                                 ...
                                 'R_SHOULDER_P': 40., 'R_SHOULDER_R': -80., 'R_SHOULDER_Y': 0., 'R_ELBOW_P': -25., \
        ...
        print("joint first Desired")
        print(self.desired['joint'])
    ...
    def customPreStep(self):
        ...
        # set torso and base references to the average of the feetget_step_index_at_time (only x components [:3] foot_pos[0:2]: for orientation)
        for link in ['torso', 'base']:
            for key in ['pos', 'vel', 'acc']:
                self.desired[link][key] = (self.desired['lfoot'][key][:3] + self.desired['rfoot'][key][:3]) / 2.

        # get torque commands using inverse dynamics --- AN- THIS LINE WILL TELL US WHAT WE NEEDDDD
        print("joint Desired")
        print(self.desired['joint'])
