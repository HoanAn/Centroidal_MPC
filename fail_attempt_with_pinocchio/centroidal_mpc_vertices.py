...
class centroidal_mpc:
  def __init__(self, initial, footstep_planner, params, CoM_ref, contact_trj_l, contact_trj_r):
    ...
    for i in range(self.N):
        cost += 1000*cs.sumsqr(self.opti_hw[:,i+1]) + \
              ...
              100*cs.sumsqr((self.opti_pos_contact_l[:,i]-self.opti_pos_contact_l_ref[:,i])*self.opti_contact_left[i]) + \
              100*cs.sumsqr((self.opti_pos_contact_r[:,i]-self.opti_pos_contact_r_ref[:,i])*self.opti_contact_right[i]) + \
  ...
  def centroidal_dynamic(self, state,CoM_ref,contact_left, contact_right,input):
    ...
    torque_l_2= contact_left*(self.skew_MX(v1l_pos-com_pos)@v1l+self.skew_MX(v2l_pos-com_pos)@v2l+self.skew_MX(v3l_pos-com_pos)@v3l+self.skew_MX(v4l_pos-com_pos)@v4l)
    torque_r_2= contact_right*(self.skew_MX(v1r_pos-com_pos)@v1r+self.skew_MX(v2r_pos-com_pos)@v2r+self.skew_MX(v3r_pos-com_pos)@v3r+self.skew_MX(v4r_pos-com_pos)@v4r)

    # Centroidal dynamic with disturbance estimator theta hat, contact dynamics
    dcom=com_vel 
    ddcom= gravity+(1/mass)*(Vl+Vr+theta_hat)
    dhw= -(torque_l_2)-(torque_r_2)
...
# let the mpc compute the state in the rest of the horizon
  ...
  def solve(self, current, t):
   ...
   # UPDATE  the contact status L/R over N
    ...
    pos_contact_ref_l = self.pos_contact_ref_l[t+idx:t+idx+self.N].T
    pos_contact_ref_r = self.pos_contact_ref_r[t+idx:t+idx+self.N].T
    ang_contact_ref_l = self.rotvec_contact_ref_l[t+idx:t+idx+self.N].T
    ang_contact_ref_r = self.rotvec_contact_ref_r[t+idx:t+idx+self.N].T
  ...
  #Some print
    ...
    print(f'v1l={self.u[0:3]}  ,  v2l={self.u[3:6]}  \n  v3l={self.u[6:9]}  ,  v4l={self.u[9:12]}')
    print(f'v1r={self.u[12:15]}   ,   v2r={self.u[15:18]}   \n   v3r={self.u[18:21]}   ,   v4r={self.u[21:24]}\n')
    ...
    if self.params['update_contact']=='YES' :
      ...
      #Update new contact position at this specific moment
        ...
        if current_contact_foot=='lfoot':
          self.footstep_planner.plan[self.footstep_planner.get_step_index_at_time(t)+1]['pos'] = self.x_collect[17:20,self.N-1]#mean the swing foot is the right one-> update the contact position of the swing foot
        else:
          self.footstep_planner.plan[self.footstep_planner.get_step_index_at_time(t)+1]['pos'] = self.x_collect[13:16,self.N-1]
  ...
  def skew_MX(self,vector):
    """
    Returns the skew-symmetric matrix associated with a 3D vector.
    
    Args:
      vector (array-like): A 3-element array or list representing the vector.
    
    Returns:
      cs.DM: A 3x3 skew-symmetric matrix.
    """
    # if len(vector) != 3:
    #   raise ValueError("Input vector must have exactly 3 elements.")
    # If vector is a CasADi MX, split it into components
    # if isinstance(vector, cs.MX):
    #     vector = cs.vertsplit(vector, 1)  # Split into a list of MX elements
    Skew=cs.MX.zeros(3,3)
    Skew[0,1]=-vector[2]
    Skew[0,2]=vector[1]
    Skew[1,2]=-vector[0] 
    Skew[1,0]=vector[2]
